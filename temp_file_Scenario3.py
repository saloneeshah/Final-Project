...
class Player:
    ...
    def choose_direction_striker(self, team, consider_direction):
        if self:
            ...
            if not consider_direction:
                ...
                n = randint(1, max(team.tendency))
                if n <= team.tendency[0]:
                    return 'Right'
                elif n <= team.tendency[0] + team.tendency[1]:
                    return 'Left'
                else:
                    return 'Middle'
    ...
    def penalty_sim(self, team, striker, print_result=False, consider_direction=False):
        # self = goalie, opponent = striker
        if striker:
            goalie_direction = self.choose_direction_goalie(consider_direction, striker)
            striker_direction = striker.choose_direction_striker(self,consider_direction)
            ...
            goalie_direction = team.choose_direction_goalie(consider_direction, striker)
            ...
            Player.record_play(team, striker, striker_direction, winner, consider_direction)
            if print_result:
                print("Team", striker.team, "kicked to the ", striker_direction, ", goal keeper jumped to the ",
                      goalie_direction, ", the winner is ", winner.name)
            if not consider_direction:
                return (team.wins_case1 / 100) * 100
            else:
                return (team.wins_case2 / 100) * 100
...
if __name__ == '__main__':
    ...
    # SCENARIO 3: goal keeper jumps in the direction the opponent team most frequently shoot in
    for match in range(10000):
        kick_taker=[]
        for i in range(len(Player.team)):
            kick_taker = Player.team
            goal_keeper = Player.keeper
            win_perc = goal_keeper.penalty_sim(kick_taker, print_result=True, consider_direction=True)
        print("Win %:", win_perc)
